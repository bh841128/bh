...
class Patient4Hospital {
	...
	//status  0:正常 1:上传  2：删除
     static public function setPatientStatusByIds($ids,$hospital_id,$status,$manager_id){
		...
		$instr=implode(",",$ids);
		$sql="update patientInfo  set status=:status ,lastmod_manager_id=:lastmod_manager_id where  hospital_id=:hospital_id and id in ($instr) and status=0  ";
		
		CUtil::logFile("=====$sql  ");
		$args=array(':status'=>$status,':hospital_id'=>$hospital_id,":lastmod_manager_id"=>$manager_id);
     	$connection = Yii::$app->db;
		$command = $connection->createCommand($sql,$args);
      ...
      static public function getPatientList($page,$hospital_id,$filter,$size=10,$status=0)
	 {
		 $ret=array(
		   "ret"=>0,
